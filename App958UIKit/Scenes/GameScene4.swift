...
class GameScene4: SKScene {
    ...
    var fullGameOver = SKSpriteNode(texture: SKTexture(imageNamed: Images.GameOver.rawValue))
    var fullGameOverNewGame = SKSpriteNode(texture: SKTexture(imageNamed: Images.GameOver.rawValue))
    var fullGameOverExit = SKSpriteNode(texture: SKTexture(imageNamed: Images.GameOverExit.rawValue))
    
    var gameOverSound = SKAudioNode(fileNamed: "CoinsFalling.mp3")
    ...
    var fullGameOverBalance = SKLabelNode(fontNamed: "FasterOne-Regular")
    ...
    var gameOverNGButton = SKSpriteNode(texture: SKTexture(imageNamed: Images.NewGame.rawValue))
    
    var background = SKSpriteNode(imageNamed: Images.Map4.rawValue)
    ...
    init(size: CGSize, gameModel: GameModel) {
        ...
        configureGameOver()
        configureFullGameOver()
        configureFallingCoins()
    }
    ...
    private func configureFallingCoins() {
        fallingCoins.position = CGPoint(x: size.width / 2, y: size.height * 1.5)
        fallingCoins.zPosition = 5
        fallingCoins.alpha = 0
        addChild(fallingCoins)
    }
    
    private func startFalling() {
        fallingCoins.alpha = 1
        let seq = SKAction.sequence([SKAction.move(to: CGPoint(x: size.width / 2, y: -size.height * 1.5), duration: 0.5), 
                                     SKAction.move(to: CGPoint(x: size.width / 2, y: size.height * 1.5), duration: 0)])
        let action = SKAction.repeat(seq, count: 14)
        gameOverSound.run(SKAction.changeVolume(to: 0.3, duration: 0))
        addChild(gameOverSound)
        fallingCoins.run(action) {
            self.fallingCoins.removeFromParent()
        }
    }
    
    private func configureFullGameOver() {
        fullGameOver.position = CGPoint(x: self.frame.midX, y: self.frame.midY)
        //gameOverBalance.position = CGPoint(x: 0, y: gameOver.size.height * 0.1)
        fullGameOver.name = "FullGameOver"
        fullGameOverBalance.text = "\(gameModel.balance)"
        fullGameOverBalance.zPosition = 9
        fullGameOverBalance.position = CGPoint(x: gameOver.size.width * 0.04, y: gameOver.size.height * 0.165)
        fullGameOverBalance.fontSize = 24
        fullGameOverBalance.horizontalAlignmentMode = .center
        fullGameOverBalance.fontColor = .white
        fullGameOver.addChild(fullGameOverBalance)
        fullGameOver.alpha = 0
        fullGameOver.zPosition = 6
        fullGameOverExit.zPosition = 9
        fullGameOverExit.name = "FullGameOverExit"
        gameOverNGButton.zPosition = 7
        gameOverNGButton.name = "GameOverNGButton"
        fullGameOverExit.position = CGPoint(x: 0, y: -fullGameOver.size.height * 0.23)
        fullGameOver.addChild(fullGameOverExit)
        fullGameOver.addChild(gameOverNGButton)
        self.addChild(fullGameOver)
    }
    ...
    private func moveBall(check: Bool = true, completion: (()->Void)? = nil) {
        ...
        ball.run(SKAction.follow(path.cgPath,
            ...
            if self.ballIndex == 8 {
                self.showFullGameOver()
            }
            completion?()
        }
    }
    
    private func showFullGameOver() {
        gameModel.saveBalance(gameModel.balance + 100)
        startFalling()
        fullGameOverBalance.text = "\(gameModel.balance)"
        exitButton.zPosition = 4 //after this window change to 6
        shadowBackground.alpha = 1
        fullGameOver.alpha = 1
    }
